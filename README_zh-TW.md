...
AI 是我的能力放大器與思考夥伴，每天都在追最新的技術與 AI 進展。
...
<!-- LIVE_STATS:START -->
> **即時數據** · 更新於 2026-07-22 00:07 ET · *由 RPi cron 自動產生*
>
> | | Trader (Alpaca 模擬) | DOWTrade (MYM 模擬) |
> |---|---|---|
> | 淨值 | $104,811.81 | $1,000,665.56 |
> | 報酬率 | +4.81% | +0.067% |
> | 持倉 | 20/20 | 空倉 |
> | 當日損益 | $+477.07 | — |
> | 總交易次數 | — | 12 |
> | 漲幅前三 | MA +8.1%, BAC +6.1%, PM +5.9% | |
> | 跌幅前三 | HD -4.2%, AVGO -0.8%, LIN -2.6% | |
...
ESP32-C6 + TMC2130 步進馬達 + LD2410 雷達 + ToF 感測器。自動寵物門，展示用了真的狗。
...
CH224K + AP63205 + ESP32-C3。footprint 畫反了，用韌體 GPIO 重映射救回來，免重洗板。
...
Gemini 分析 → HuggingFace 情緒分析 → Claude 風控閘門 → Alpaca 模擬下單。每 30 分鐘掃 30 支大型股，trailing stop + AI 出場。
...
ESP32 + ESP8266 透過 ESP-NOW 通訊，NeoPixel、I2S 音效、MPU-6050 動作感測。
...
ESP8266 + CC1101 868 MHz → 嗅探專有 RF 訊號 → 整合進 Home Assistant 控制通風。
...
| [Kitchen](https://github.com/Hydr0neFN/Kitchen) | ESP8266 × 2：LD2410B 存在偵測 → HomeKit、Sonoff Dual R3 繼電器 |
