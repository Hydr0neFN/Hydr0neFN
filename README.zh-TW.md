...
<!-- LIVE_STATS:START -->
> **即時數據** · 更新於 2026-09-24 17:15 ET · *由 RPi cron 自動產生*
>
> | | Trader (Alpaca 模擬) | DOWTrade (MYM 模擬) |
> |---|---|---|
> | 淨值 | $106,499.26 | $1,001,806.92 |
> | 報酬率 | +6.50% | +7.23% |
> | 淨損益 | — | $+1,806.92 |
> | 持倉 | 24/24 | SHORT 3 |
> | 當日損益 | $+145.10 | — |
> | 總交易次數 | — | 90 |
> | 漲幅前三 | AMD +24.6%, META +26.0%, TMO +11.6% | |
> | 跌幅前三 | CVX -2.3%, NOW -2.1%, KO -1.8% | |
>
> DOWTrade 報酬率以 $25,000 風險資本計算，而非模擬帳戶的 $1M 券商預設值 — 每筆風險預算僅 $250，對一百萬報價沒有意義。
> 對照組：同一批 15m bar、同樣的 2×ATR 停損與部位大小，只把 LLM 決策換成 EMA(9/21) 交叉 → **$-4,968.49 / 235 筆**（勝率 23.8%，平均 -0.059R）。把 LLM 版本用**同一套成本模型**重算（每筆一跳逆向滑價 + 手續費）為 **$+649.44 / 90 筆**。上表的淨損益是歷史帳載值，未含成本模型 — 成本模型自 2026-09-09 起才對新倉位生效。
