...
打孔而非全密閉設計；底下的土壤是否依然能變乾，是透過記錄的土壤電壓來驗證，
而非憑空假設。
...
<!-- LIVE_STATS:START -->
> **即時數據** · 更新於 2026-09-22 17:15 ET · *由 RPi cron 自動產生*
>
> | | Trader (Alpaca 模擬) | DOWTrade (MYM 模擬) |
> |---|---|---|
> | 淨值 | $106,650.53 | $1,001,046.90 |
> | 報酬率 | +6.65% | +4.19% |
> | 淨損益 | — | $+1,046.90 |
> | 持倉 | 24/24 | SHORT 2 |
> | 當日損益 | $+238.54 | — |
> | 總交易次數 | — | 90 |
> | 漲幅前三 | AMD +23.0%, META +19.6%, TMO +8.3% | |
> | 跌幅前三 | CVX -3.6%, CRM -2.9%, NOW -2.6% | |
>
> DOWTrade 報酬率以 $25,000 風險資本計算，而非模擬帳戶的 $1M 券商預設值 — 每筆風險預算僅 $250，對一百萬報價沒有意義。
> 對照組：同一批 15m bar、同樣的 2×ATR 停損與部位大小，只把 LLM 決策換成 EMA(9/21) 交叉 → **$-4,745.45 / 234 筆**（勝率 23.9%，平均 -0.054R）。把 LLM 版本用**同一套成本模型**重算（每筆一跳逆向滑價 + 手續費）為 **$+649.94 / 90 筆**。上表的淨損益是歷史帳載值，未含成本模型 — 成本模型自 2026-09-09 起才對新倉位生效。
