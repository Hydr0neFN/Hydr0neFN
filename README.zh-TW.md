...
<!-- LIVE_STATS:START -->
> **即時數據** · 更新於 2026-09-21 17:15 ET · *由 RPi cron 自動產生*
>
> | | Trader (Alpaca 模擬) | DOWTrade (MYM 模擬) |
> |---|---|---|
> | 淨值 | $106,329.73 | $1,001,053.94 |
> | 報酬率 | +6.33% | +4.22% |
> | 淨損益 | — | $+1,053.94 |
> | 持倉 | 24/24 | 空倉 |
> | 當日損益 | $+1,186.07 | — |
> | 總交易次數 | — | 90 |
> | 漲幅前三 | AMD +21.8%, META +20.1%, TMO +8.4% | |
> | 跌幅前三 | CVX -3.0%, KO -2.6%, NOW -2.1% | |
>
> DOWTrade 報酬率以 $25,000 風險資本計算，而非模擬帳戶的 $1M 券商預設值 — 每筆風險預算僅 $250，對一百萬報價沒有意義。
> 對照組：同一批 15m bar、同樣的 2×ATR 停損與部位大小，只把 LLM 決策換成 EMA(9/21) 交叉 → **$-4,656.82 / 233 筆**（勝率 24.0%，平均 -0.052R）。把 LLM 版本用**同一套成本模型**重算（每筆一跳逆向滑價 + 手續費）為 **$+650.94 / 90 筆**。上表的淨損益是歷史帳載值，未含成本模型 — 成本模型自 2026-09-09 起才對新倉位生效。
